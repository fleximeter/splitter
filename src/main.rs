...
fn process(files: &Vec<(String, String, String)>, max_num_frames: usize, max_num_threads: usize, fade_in: bool) {
        ...
        pool.execute(move || {
            ...
            match aus::read(&file) {
                Ok(mut audio) => {
                    ...
                    let mut num_files = audio.num_frames / max_num_frames;
                    if num_files * max_num_frames < audio.num_frames {
                        num_files += 1;
                    }
                    
                    // Fill missing information if necessary
                    ...
                    for i in 0..num_files {
                        ...
                        let start_idx = i * max_num_frames;
                        let end_idx = usize::min(start_idx + max_num_frames, audio.num_frames);
                        let mut new_samples: Vec<Vec<f64>> = Vec::new();
                        for j in 0..audio.num_channels {
                            new_samples.push(Vec::new());
                            for k in start_idx..end_idx {
                                new_samples[j].push(audio.samples[j][k]);
                            }
                        }
                        if fade_in {
                            for j in 0..audio.num_channels {
                                aus::operations::fade_in(&mut new_samples[j], aus::WindowType::Hanning, 1000);
                                aus::operations::fade_out(&mut new_samples[j], aus::WindowType::Hanning, 1000);
                            }
                        }
                        let new_audio = aus::AudioFile::new(audio.audio_format, audio.sample_rate, new_samples);
                        match aus::write(&new_file_name, &new_audio) {
                            Ok(_) => (),
                            Err(err) => match err {
...
fn main() {
    ...
    let args = match validate_args(std::env::args().collect()) {
        Some(x) => x,
        None => {
            println!("Usage:\n-f folder_name -n num_frames -t num_threads\nOptional: include the -d flag to delete the original files when done, and/or the -i flag to add a brief fade in/fade out to the new files.");
            return;
        }
    };
    ...
    println!("Splitting {} files...", files.len());
    process(&files, args.num_frames, args.num_threads, args.fade_in);
